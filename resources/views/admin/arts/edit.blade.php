        ...
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-xl overflow-hidden p-6 lg:p-8">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.arts.update', $art->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="user_id" class="block font-semibold text-sm text-zinc-700 mb-1">Pemilik Karya (User)</label> 
                            <select id="user_id" name="user_id" class="block mt-1 w-full px-4 py-3 border border-[#163832] bg-[#DAF1DE] rounded-lg shadow-inner-sm focus:outline-none focus:ring-2 focus:ring-[#235347] focus:border-[#235347] transition duration-200 ease-in-out text-[#051F20]" required>
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ old('user_id', $art->user_id) == $user->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-6"> 
                            ...
                            <input id="image" type="file" name="image" class="block mt-1 w-full text-sm text-[#051F20] file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#DAF1DE] file:text-[#0B2B26] hover:file:bg-[#8E8B9B] hover:file:text-white file:transition file:duration-150 file:ease-in-out cursor-pointer">
                            @error('image')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                            @if ($art->image_path)
                                <p class="text-sm text-gray-600 mt-2">Gambar saat ini:</p>
                                <img src="{{ Storage::url($art->image_path) }}" alt="{{ $art->title }}" class="w-32 h-32 object-cover rounded-md mt-1 shadow-sm border border-[#DAF1DE]"> {{-- Border dan shadow untuk gambar --}}
                            @endif
                        </div>
