            ...
            <div class="bg-white rounded-lg shadow-xl overflow-hidden p-6 lg:p-8">
                <div class="p-6 text-gray-900">
                    <div class="mb-6 flex justify-between items-center">
                        <h3 class="text-2xl font-bold text-[#051F20]">Daftar Semua Karya Seni</h3>
                        <a href="{{ route('admin.arts.create') }}" class="inline-flex items-center px-4 py-2 bg-[#163832] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-[#0B2B26] focus:outline-none focus:ring-2 focus:ring-[#235347] focus:ring-offset-2 transition ease-in-out duration-150">
                            <i class="fas fa-plus mr-2"></i> Tambah Artwork Baru
                        </a>
                    </div>
                        ...
                        <table class="min-w-full rounded-lg overflow-hidden"> 
                            <thead class="bg-[#235347]">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Diunggah Oleh</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Gambar</th>
                            ...
                            <tbody class="bg-white">
                                @forelse ($arts as $art)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900 max-w-xs truncate">{{ $art->title }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-900 max-w-xs truncate">{{ Str::limit($art->description, 50) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $art->user->name ?? '-' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            <img src="{{ Storage::url($art->image_path) }}" alt="{{ $art->title }}" class="w-16 h-16 object-cover rounded-md shadow-sm">
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center space-x-2">
                                            @can('manage-all-arts')
                                                <a href="{{ route('admin.arts.edit', $art->id) }}" class="text-[#235347] hover:text-[#163832] transition duration-150">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <form action="{{ route('admin.arts.destroy', $art->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus karya seni ini secara permanen?');" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            @else
                                                <span class="text-gray-400 ml-2 text-xs"><i class="fas fa-ban mr-1"></i> Tidak ada izin</span>
                                            @endcan
                                        </td>
                                    </tr>
                                @empty
