        ...
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8"> {{-- Ukuran tetap max-w-xl --}}
            <div class="bg-white rounded-lg shadow-xl overflow-hidden p-6 lg:p-8"> {{-- Kartu form --}}
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.user.update', $user->id) }}"> {{-- Pastikan route benar, gunakan $user->id --}}
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4"> {{-- Margin bottom --}}
                            ...
                            @if ($user->role === 'admin')
                                <input id="role" class="block mt-1 w-full px-4 py-3 bg-[#8E8B9B] border border-[#235347] rounded-lg shadow-sm cursor-not-allowed text-[#051F20] font-semibold" type="text" value="Admin" disabled /> {{-- Warna input disabled & teks disesuaikan --}}
