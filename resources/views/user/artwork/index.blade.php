            ...
            <div class="bg-white rounded-lg shadow-xl overflow-hidden p-6 lg:p-8"> {{-- Kartu utama --}}
                <div class="p-6 text-gray-900">
                    <div class="mb-6 flex justify-between items-center">
                        <h3 class="text-2xl font-bold text-[#051F20]">Karya Seni Saya</h3> {{-- Warna teks judul --}}
                        <a href="{{ route('user.artwork.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-[#163832] to-[#235347] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-wider hover:from-[#0B2B26] hover:to-[#163832] focus:outline-none focus:ring-2 focus:ring-[#8E8B9B] focus:ring-offset-2 transition ease-in-out duration-150"> {{-- Tombol Unggah --}}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6"> {{-- Grid responsif --}}
                            ...
                            <div class="relative bg-gradient-to-br from-[#163832] to-[#0B2B26] rounded-xl shadow-lg overflow-hidden group transform hover:scale-105 transition duration-300 ease-in-out cursor-pointer border border-[#DAF1DE]">
                                <a href="{{ route('user.gallery.show', $art->id) }}"> {{-- Link ke detail karya --}}
                                    ...
                                    <div class="p-4 relative z-10 text-white">
                                        ...
                                        <p class="text-[#DAF1DE] text-sm mb-2">Diunggah pada: {{ $art->created_at->format('d M Y') }}</p>
                                    </div>
                                </a>
                                {{-- Tombol Edit dan Hapus --}}
                                <div class="p-4 pt-0 relative z-10 flex space-x-2 justify-end"> {{-- Padding disesuaikan --}}
                                    <a href="{{ route('user.artwork.edit', $art->id) }}" class="inline-flex items-center px-3 py-1 bg-gradient-to-r from-[#DAF1DE] to-[#8E8B9B] border border-transparent rounded-md font-semibold text-xs text-[#0B2B26] uppercase tracking-wider hover:from-[#8E8B9B] hover:to-[#235347] focus:outline-none focus:ring-2 focus:ring-[#DAF1DE] focus:ring-offset-2 transition ease-in-out duration-150">
                                        Edit
                                    </a>
                                    <form action="{{ route('user.artwork.destroy', $art->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus karya ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center px-3 py-1 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-wider hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
                        ...
                        @empty
                            <p class="col-span-full text-center text-gray-600 text-lg py-10">Anda belum mengunggah karya seni apapun. <a href="{{ route('user.artwork.create') }}" class="text-[#235347] font-semibold hover:text-[#163832] transition duration-150">Unggah sekarang!</a></p>
                        @endforelse
                    </div>
