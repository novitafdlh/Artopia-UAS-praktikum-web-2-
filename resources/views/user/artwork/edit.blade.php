        ...
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8"> {{-- Ukuran tetap max-w-xl --}}
            <div class="bg-white rounded-lg shadow-xl overflow-hidden p-6 lg:p-8"> {{-- Kartu form --}}
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('user.artwork.update', $art->id) }}" enctype="multipart/form-data"> {{-- Gunakan $art->id untuk route --}}
                        @csrf
                        @method('PUT')

                        <div class="mb-4"> {{-- Margin bottom --}}
                            <label for="title" class="block font-semibold text-sm text-zinc-700 mb-1">Judul Karya</label> {{-- Label style --}}
